#include "../algorithms/cryptography/sha512.c"
	...
	while (*line_begin)
	{
		...
		*line_end = 0;
		...
		if (len && msg && ref_digest)
		{
			...
			assert(strlen((char *) ref_digest) == 128);
			...
			sha512_init(&sha512);
			sha512_add(&sha512, msg, length);
			sha512_finish(&sha512, digest);
...
{
	run_tests("data/SHA512ShortMsg.txt");
	run_tests("data/SHA512LongMsg.txt");
